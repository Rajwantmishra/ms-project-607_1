...
This was a group project with the following contributors:
...
In this project, we're given a text file with chess tournament results where the information has some structure. Generates a .CSV file (that could for example be imported into a SQL database) with the following information for all of the players:
...
cd2 <-read_lines("playerdata.txt",skip = 1)
...
#### Key Methods Used To Restructed the .txt file into the Dataframe {.tabset .tabset-fade .tabset.-pills}

***
##### The R Package
`stringr` was the main package needed to solve this problem, while any other package were utilized to create a presentable R markdown page.
...
# Creating a sample to work with small data 
tempdata <- unlist(str_replace_all(cd2,"(-{89})","NA"))[c(4:5,7:8)]
#player -ID
playerID <- trimws(unlist(str_extract_all(cd2_data," \\d{1,2} ")))
...
## extractt the data from column 1 and column 2
...
# calculated by using the pre-tournament opponents' ratings
...
for (i in 1:length(ChessTourn$PreRating)){
    ...
    PreRatingAvg <- round(TotalRating / NumOppt, digits = 0)   
    }
